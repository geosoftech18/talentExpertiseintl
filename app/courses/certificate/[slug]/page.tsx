...
export default function CertificateCoursesPage() {
  ...
  const certificateId = params.slug as string
  ...
  // Fetch certificates first to get the name
  useEffect(() => {
  ...
  // Filter courses by certificate and exclude expired courses
  const certificateCourses = useMemo(() => {
    const today = new Date()
    today.setHours(0, 0, 0, 0) // Reset time to start of day for accurate comparison
    
    return allCourses.filter(course => {
      // Must have matching certificate
      ...
      if (!certificateIdsAsStrings.includes(String(certificateId))) return false
  ...
  }, [allCourses, certificateId])

  // Extract unique values for filters
  const availableCategories = useMemo(() => {
    return Array.from(new Set(certificateCourses.map(c => c.category).filter(Boolean))).sort() as string[]
  }, [certificateCourses])

  const availableVenues = useMemo(() => {
    return Array.from(new Set(certificateCourses.map(c => c.venue).filter(Boolean))).sort() as string[]
  }, [certificateCourses])
  ...
  const filteredCourses = useMemo(() => {
    let filtered = [...certificateCourses]
    ...
    return filtered
  }, [certificateCourses, selectedCategory, selectedVenue, selectedMonth, selectedYear])
        ...
        {loading || loadingCertificates ? (
              ...
              <table className="w-full border-collapse min-w-[800px]">
                <thead className="bg-gradient-to-r from-[#0A3049] to-[#0A3049] text-white">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs sm:text-sm font-semibold">CourseDate</th>
          ...
          <div className="bg-white rounded-lg shadow-lg text-center py-12">
            ...
            <p className="text-sm text-slate-600">
              {certificateName 
                ? `No courses available for ${certificateName} with the selected filters.`

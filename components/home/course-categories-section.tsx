  ...
  useEffect(() => {
    ...
    const fetchCategoryCourses = async () => {
      ...
      try {
        ...
        // Map category title to database category name
        // The database category should match the category title
        const categoryName = category.title
        
        const response = await fetch(`/api/courses?category=${encodeURIComponent(categoryName)}&limit=50`)
        const result = await response.json()

        if (result.success && result.data) {
          setCategoryCourses(result.data)
        } else {
          setCategoryCourses([])
        }
      } catch (error) {
                  ...
                  {(() => {
                    ...
                    return (
                      ...
                      >
                              ...
                              <p className="text-sm sm:text-base text-gray-600">
                                {loadingCourses 
                                  ...
                                  : categoryCourses.length > 0 
                                    ? `Found ${categoryCourses.length} upcoming ${category.title.toLowerCase()} training program${categoryCourses.length !== 1 ? 's' : ''}`
                        ...
                        {/* Courses Slider */}
                        {!loadingCourses && categoryCourses.length > 0 && (
                          <div className="relative">
                                    ...
                                    <div className="flex items-start justify-between mb-3 sm:mb-4">
                                      ...
                                      <div className="text-right">
                                        <div className="text-lg sm:text-xl font-bold text-gray-900">
                                          ${course.price ? course.price.toLocaleString() : 'TBA'}
                                        </div>
                                        {course.venue && (
                        ...
                        {!loadingCourses && categoryCourses.length === 0 && (
                          <div className="text-center py-12">
                            ...
                            <p className="text-gray-600 mb-4">No upcoming courses available for this category.</p>
                            <Button
                              variant="outline"
                              onClick={() => {

...
const emptyForm = { name: "", imageUrl: "", url: "#", sortOrder: 0, status: "Active" }
...
export default function AffiliationsAdmin() {
  ...
  const [editing, setEditing] = useState<AffiliationItem | null>(null)
  const [form, setForm] = useState(emptyForm)
  const [saving, setSaving] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<AffiliationItem | null>(null)
  const [seeding, setSeeding] = useState(false)

  const fetchItems = async () => {
    try {
      setLoading(true)
      let res = await fetch("/api/admin/affiliations?limit=1000")
      let result = await res.json()
      let data = result.success ? result.data || [] : []

      if (data.length === 0) {
        const seedRes = await fetch("/api/admin/affiliations", {
          method: "POST",
        ...
        if (seedResult.success && Array.isArray(seedResult.data)) {
          ...
          res = await fetch("/api/admin/affiliations?limit=1000")
  ...
  const openEdit = (item: AffiliationItem) => {
    setEditing(item)
    setForm({
      name: item.name,
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/affiliations?id=${deleteTarget.id}`, { method: "DELETE" })
      const result = await res.json()
  ...
  const handleSeed = async () => {
    if (!confirm("Import the current homepage affiliations into the database? (Only if the list is empty)")) return
    try {
      setSeeding(true)
      const res = await fetch("/api/admin/affiliations", {
        method: "POST",
  ...
  return (
    <div className="p-8 space-y-6 theme-bg">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-bold theme-text mb-2">Affiliations</h1>
          <p className="theme-muted">Manage logos in Affiliations &amp; Accreditations</p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="theme-card rounded-xl p-10 text-center space-y-3">
          <p className="theme-muted">No affiliations in the database yet. Homepage still shows built-in defaults.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filtered.map((item) => (
            <div key={item.id} className="theme-card rounded-xl p-3 border border-border space-y-2">
              ...
              <p className="text-[10px] theme-muted truncate" title={item.url || ""}>{item.url || "#"}</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Affiliation" : "Add Affiliation"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div>
              ...
              {form.imageUrl && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={form.imageUrl} alt="Preview" className="mt-2 h-20 object-contain border rounded" />
              )}
            </div>
            <div>
              <label className="text-sm theme-muted mb-1 block">Link URL</label>
              <Input
                value={form.url}
                onChange={(e) => setForm((p) => ({ ...p, url: e.target.value }))}
                placeholder="/courses/certificate/... or https://..."
              />
            </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove &quot;{deleteTarget?.name}&quot; from the homepage.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
